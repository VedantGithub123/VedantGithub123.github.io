...
def getInfo():
  ...
  data = df.loc[(df['Year'] == int(data["year"]))
                & (df['Month'] == data["month"]) &
                (df['Day'] == int(data["day"]))].to_dict()
  for i in data:
...
def getMonthFromYear():
  data = request.get_json()
  return json.dumps(
      {"months": list(set(df.loc[df['Year'] == int(data["year"]), 'Month']))})
...
def getDayFromMonthYear():
  ...
  return json.dumps({
      "days":
      list(
          set(df.loc[(df['Year'] == int(data["year"])) &
                     (df['Month'] == data["month"]), 'Day']))
  })
